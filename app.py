...
def ierParser():
    ...
    if req.status_code == 200:
        ...
        db = CRUD()
        for i in range(len(result)):
            db.insertIerData(name=result[i]['name'],erm=result[i]['erm'],unit=result[i]['unit'],time=result[i]['time'])
...
def doesParser():
    ...
    db = CRUD()
    for i in range(len(result)):
        db.insertDoseData (result[i]['id'] , result[i]['name'],result[i]['modelcode'], result[i]['time'], result[i]['unit'] , result[i]['doserate'] , result[i]['latitude'] ,result[i]['longitude'] ,result[i]['rainfall'],result[i]['temperature'], result[i]['windspeed'] , result[i]['winddirection'])
